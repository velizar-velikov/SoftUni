...
const endpoints = {
    ...
    own: (postId, userId) => `/data/donations?where=postId%3D%22${postId}%22%20and%20_ownerId%3D%22${userId}%22&count`,
...
describe('E2E tests', function () {
    ...
    describe('CRUD [ 50 Points ]', () => {
        ...
        it('Check details information [ 5 Points ]', async () => {
            ...
            const title = await page.$$eval('#details-page .post-title', (t) => t.map((s) => s.textContent));
            const image = await page.$$eval('#details-page .post-image', (t) => t.map((s) => s.src));
            const description = await page.$$eval('#details-page .post-description', (t) => t.map((s) => s.textContent));
            const address = await page.$$eval('#details-page .post-address', (t) => t.map((s) => s.textContent));
            const phone = await page.$$eval('#details-page .post-number', (t) => t.map((s) => s.textContent));
