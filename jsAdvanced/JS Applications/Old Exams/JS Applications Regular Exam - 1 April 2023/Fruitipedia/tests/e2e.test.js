...
const endpoints = {
    ...
    own: (fruitId, userId) => `/data/fruits?where=fruitId%3D%22${fruitId}%22%20and%20_ownerId%3D%22${userId}%22&count`,
};
...
describe('E2E tests', function () {
    ...
    describe('Navigation bar [ 5 Points ]', () => {
        ...
        it('Guest user should see correct navigation [ 2.5 Points ]', async () => {
            ...
            expect(await page.isVisible('nav >> text=Fruits'), 'Dashboard is not visible').to.be.true;
            expect(await page.isVisible('nav >> text=Add Fruit'), 'Create is visible').to.be.false;
    ...
    describe('Fruits Page [ 15 Points ]', () => {
        ...
        it('Check fruits page with 2 offers [ 2.5 Points ]', async () => {
            ...
            const names = await page.$$eval('.fruit .title', (t) => t.map((s) => s.textContent));
        ...
        it('Check fruit page Info [ 5 Points ]', async () => {
            ...
            await page.waitForSelector('#dashboard');
            const names = await page.$$eval('.fruit .title', (t) => t.map((s) => s.textContent));
            const description = await page.$$eval('.fruit .description', (t) => t.map((s) => s.textContent));
    ...
    describe('CRUD [ 50 Points ]', () => {
        ...
        it('Check More Info information [ 5 Points ]', async () => {
            ...
            const nutrition = await page.$$eval('#details-wrapper #info-wrapper #details-description #details-nutrition', (t) =>
                t.map((s) => s.textContent)
            );
            const description = await page.$$eval('#details-wrapper #info-wrapper #details-description p', (t) =>
                t.map((s) => s.textContent)
            );
